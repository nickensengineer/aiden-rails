...
document.addEventListener('turbolinks:load', function () {
    ...
    projectForm.addEventListener('ajax:error', function (xhr, status, err) {
        ...
        project.Name.classList.add('is-valid')

        var errorNode = document.createElement('div')
        var errorTextNode = document.createTextNode('Name must note be null')
        error.Node.classList.add('invalid-feedback')
        error.Node.appendChild(errorTextNode)
